...
func main() {
	// launcherBaseURL: where THIS LAUNCHER's own .exe/.sha256 live, so it can
	// self-update (see doUpdate below). This is unrelated to KrayAcc - it's
	...
	launcherBaseURL := "http://187.127.62.166:8090/launcher_client/"
	...
	parallel := 64
	// clientExecutable is the filename of the GAME client's own .exe inside
	...
	if err := viper.ReadInConfig(); err == nil {
		...
		manifestURL = viper.GetString("manifest_url")
		if viper.GetString("executable") != "" {
			clientExecutable = viper.GetString("executable")
		}
	} else {
		viper.Set("parallel", parallel)
		viper.Set("launcher_base_url", launcherBaseURL)
		viper.Set("manifest_url", manifestURL)
		viper.Set("executable", clientExecutable)
	}
	...
	if err := doUpdate(logger, launcherBaseURL+executable); err != nil {
		logger.Errorf("Failed to update: %v", err)
		os.Exit(1)
	}

	app := NewApp(logger, manifestURL, appName, parallel)
